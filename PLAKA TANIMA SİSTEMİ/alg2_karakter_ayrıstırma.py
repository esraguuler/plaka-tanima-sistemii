...
import numpy as np
from plakatespiti import plaka_konum_don
...
plake_resim = cv2.morphologyEx(plaka_resim,cv2.MORPH_OPEN,kernel,iterations=1)

plt.title("Gürültü yok edilmiş")
plt.imshow(plaka_resim,cmap="gray")
plt.show()

cnt = cv2.findContours(plaka_resim,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
cnt = cnt[0]
